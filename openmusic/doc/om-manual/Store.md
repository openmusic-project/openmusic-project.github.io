Navigation : [Previous](SlotsBox "\(Slots\)") | [Next](InterfaceBoxes "\(Interface Boxes\)")
...
The icon of a store object ![](../res/store_icon.png) represents a memory space.
...
**Using a Slots Box and a Store Instance - or Global Variable:**
...
3. Connect the "self" output of the  store instance  to the "self" input of the store slots box .
...
A global variable can be edited, from the **Globals** window, or from a patch editor. Double click on the icon of a store object ![](../res/store_icon.png) to open its editor.
...
This means the value of this variable is "nil". To modify this value, the user must resort to the procedures described above.

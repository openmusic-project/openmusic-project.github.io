Navigation : [Previous](InfiniteLoops "\(Infinite Loops\)") | [Next](Collect "\(Collect\)")

# Accumulators

## Function

At each step of an iteration, resulting data can be memorized and stored with the **OMLoop accumulators** . Accumulators execute storage or other operation
...
## Inputs and Outputs
  ...
  * All accumulators but **acucm** have a single input : this input is used for **collecting the successive values** during a loop.
  ...
  * All accumulator have three outputs, allowing to access their current "memory"  :
    ...
    * second output : **returns the current state**  of the accumulator's memory,
